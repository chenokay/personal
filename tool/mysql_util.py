...
本模块提供了一组全局的mysql连接池，可以在一个py文件中import本模块并初始化，在
其他的py文件import本模块，对数据库进行访问。

import domob_pyutils.mysql_util as MU
MU.registerConnection('online_db', host="192.168.1.200", port=3306,
	user='domob', passwd='domob', db='online_db', pool_size=8)
...
class MySQLConnPool(object):
	...
	def getConn(self):
		...
		while len(self.__connections) == 0 and self.__conn_num >= self.pool_size:
			_logger.warn("handle is used up[connections:%d][conn_num:%d]" %(len(self.__connections), self.__conn_num))
